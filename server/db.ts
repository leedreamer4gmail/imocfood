...
import { InsertNewsArticle, InsertUser, newsArticles, users } from "../drizzle/schema";
...
/** Get all published articles (for public News page), newest first */
...
/** Get all articles (for admin CMS), newest first */
...
export async function createArticle(data: InsertNewsArticle) {
  const db = await getDb();
...
export async function updateArticle(id: number, data: Partial<InsertNewsArticle>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
