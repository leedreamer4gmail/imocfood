...
var CHROMA_API_KEY, CHROMA_TENANT, CHROMA_DATABASE, COLLECTION_NAME, BASE_URL, DUMMY_EMBEDDING;
var init_chromaDb = __esm({
  "server/chromaDb.ts"() {
    "use strict";
    CHROMA_API_KEY = process.env.CHROMA_API_KEY ?? "";
    CHROMA_TENANT = process.env.CHROMA_TENANT ?? "";
    CHROMA_DATABASE = "imocfood";
  ...
  try {
    const response = await invokeLLM({
      messages: [
        {
          role: "system",
          content: "You are a professional translator specializing in Chinese to English translation for a food/meat products company. Translate the provided Chinese text to natural, professional English. Return ONLY the translated text, no explanations or extra content."
        },
        {
          role: "user",
          content: chineseText
